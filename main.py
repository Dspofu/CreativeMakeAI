...
qtdImg = 1
splash = None
torch, select_model, initialization_model, list_lora, select_lora, unload_lora, load_plugins, pynvml = None, None, None, None, None, None, None, None

# Carregar funções e pacotes
def loading_functions(status, progress):
  global torch, select_model, initialization_model, list_lora, select_lora, unload_lora, load_plugins, pynvml
  status.after(0, lambda: status.configure(text='Carregando função "config"'))
  import config
  status.after(0, lambda: status.configure(text='Carregando função "load_torch"'))
  from loader_packages import load_torch
  torch = load_torch()
  status.after(0, lambda: status.configure(text='Carregando função "load_gpu_monitor"'))
  from loader_packages import load_gpu_monitor
  pynvml = load_gpu_monitor()
  status.after(0, lambda: status.configure(text='Carregando função "load_image_renderer"'))
  from loader_packages import load_image_renderer
  initialization_model = load_image_renderer()
  status.after(0, lambda: status.configure(text='Carregando função "load_lora_system"'))
  from loader_packages import load_lora_system
  list_lora, select_lora, unload_lora = load_lora_system()
  status.after(0, lambda: status.configure(text='Carregando função "load_model_selector"'))
  from loader_packages import load_model_selector
  select_model = load_model_selector()
  status.after(0, lambda: status.configure(text='Carregando função "load_plugins_system"'))
  from loader_packages import load_plugins_system
  load_plugins = load_plugins_system()
  status.after(0, lambda: status.configure(text='Carregando função "popup"'))
  import src.functions.popup
  def initialization_window():
    status.configure(text='Iniciando Janela Principal')
    for widget in splash.winfo_children():
      widget.destroy()
    splash.overrideredirect(False)
    progress.stop()
    main_window()
  status.after(0, initialization_window)


# Janela Principal do Usuario
def main_window():
  global torch, select_model, initialization_model, list_lora, select_lora, unload_lora, load_plugins, pynvml, splash
  from config import Window, Colors, os, data, alert, config
  from src.functions.loading import init_progress
  from painel_config import open_painel

  init_progress()
  global qtdImg

  config.window = splash
  config.window.geometry(f"520x780+{int((config.window.winfo_screenwidth() / 2) - (520 / 2))}+{int((config.window.winfo_screenheight() / 2) - (780 / 2))}")
  config.window.title(Window.winTitle)
  config.window.resizable(False, False)
  config.window.configure(fg_color=Colors.COR_FRAME)
  ...
  def reload_plugins():
    ...
    for plugin in plugins:
      try:
        plugin.creative_module(config.window, tab_view) 
  ...
  status_bar = ctk.CTkFrame(config.window, height=30, fg_color="#14141f", corner_radius=0)
  ...
  reload_container = ctk.CTkFrame(config.window, fg_color="transparent")
  ...
  # Prompt
  prompts_container = ctk.CTkFrame(config.window, fg_color="transparent")
  ...
  tab_view = ctk.CTkTabview(config.window, width=480, height=300, fg_color=Colors.COR_INPUT, segmented_button_fg_color=Colors.COR_FRAME, segmented_button_selected_color=Colors.COR_BOTAO)
  ...
  # Botão de Gerar
  footer_dock = ctk.CTkFrame(config.window, height=80, fg_color=Colors.COR_FRAME)
  ...
  generate_button = ctk.CTkButton(action_area, text="GERAR IMAGEM", command=lambda: initialization_model(generate_button, temperature_label, f"{width_entry.get()}x{height_entry.get()}", prompt_entry, negative_prompt_entry, steps, cfg, seed_entry, lora_listbox, qtdImg=qtdImg), state="disabled", font=("Roboto", 15, "bold"), fg_color=Colors.COR_BOTAO, hover_color=Colors.COR_BOTAO_HOVER, height=40, corner_radius=8)
  generate_button.pack(side="left", fill="x", expand=True, padx=(0, 5))

  config.window.withdraw()
  open_painel(config.window)
  config.window.update()
  config.window.deiconify()
  config.window.mainloop()


# Janela de Carregamento Inicial
splash = ctk.CTk()
splash.title("Loading Packages")

# Centralizar Splash
w_width, w_height = 300, 150
s_width, s_height = splash.winfo_screenwidth(), splash.winfo_screenheight()
x, y = int((s_width/2) - (w_width/2)), int((s_height/2) - (w_height/2))
splash.geometry(f"{w_width}x{w_height}+{x}+{y}")
splash.overrideredirect(True)

# Layout da Splash
frame = ctk.CTkFrame(splash, corner_radius=10, fg_color="#1a1a1a", border_width=1, border_color="#333")
frame.pack(fill="both", expand=True)

lbl_title = ctk.CTkLabel(frame, text="Aguarde o processo", font=("Roboto", 18, "bold"), text_color="white")
lbl_title.pack(pady=(30, 10))

lbl_status = ctk.CTkLabel(frame, text="Iniciando", font=("Roboto", 12), text_color="#aaaaaa")
lbl_status.pack(pady=5)

progress = ctk.CTkProgressBar(frame, width=200, height=8, progress_color="#EFFF3F")
progress.pack(pady=10)
progress.configure(mode="indeterminate")
progress.start()

threading.Thread(
  target=loading_functions,
  args=(lbl_status, progress,),
  daemon=True
).start()
splash.mainloop()