...
from src.modules.viwer_image import viwerImage
...
from src.modules.model import select_model
...
def active_benchmark_sequence():
  if not torch.cuda.is_available():
    ...
    config.benchmark_sequence = not config.benchmark_sequence
...
alert_check = ctk.CTkCheckBox(temp_container, text="Alert", command=active_temp_alert, font=("Roboto", 10), text_color="#AAAAAA", fg_color=COR_BOTAO, hover_color=COR_BOTAO_HOVER, width=16, height=16, corner_radius=4)
alert_check.pack(side="left")

if config.limit_temp:
  alert_check.select()
else:
...
reload_btn = ctk.CTkButton(reload_container, text="Reload Plugins", command=reload_plugins, font=("Roboto", 11, "bold"), width=10, height=25, fg_color=COR_BOTAO, hover_color=COR_BOTAO_HOVER, corner_radius=6)
...
prompt_entry = ctk.CTkTextbox(prompts_container, height=80, font=("Roboto", 13), fg_color=COR_INPUT, text_color=COR_TEXTO, corner_radius=6, border_width=0)
...
negative_prompt_entry = ctk.CTkTextbox(prompts_container, height=100, font=("Roboto", 13), fg_color=COR_INPUT, text_color="#DDDDDD", corner_radius=6, border_width=0)
negative_prompt_entry.insert("0.0", negative_prompt)
negative_prompt_entry.pack(fill="x", pady=2)

tab_view = ctk.CTkTabview(window, width=480, height=300, fg_color=COR_INPUT, segmented_button_fg_color=COR_FRAME, segmented_button_selected_color=COR_BOTAO)
...
model_button = ctk.CTkButton(model_row, text="Selecionar Modelo (.safetensors)", command=lambda: select_model(model_button, generate_button, lora_listbox, model_lora, loaded_loras), font=("Roboto", 12), fg_color=COR_BOTAO, hover_color=COR_BOTAO_HOVER, height=30, corner_radius=6)
...
width_entry = ctk.CTkEntry(res_inputs, font=("Roboto", 12), fg_color=COR_FRAME, border_width=0, corner_radius=4, justify="center")
...
height_entry = ctk.CTkEntry(res_inputs, font=("Roboto", 12), fg_color=COR_FRAME, border_width=0, corner_radius=4, justify="center")
...
seed_entry = ctk.CTkEntry(col_seed, font=("Roboto", 12), fg_color=COR_FRAME, border_width=0, corner_radius=4)
...
steps_entry = ctk.CTkEntry(lbl_steps, width=40, height=20, fg_color=COR_FRAME, border_width=0, justify="center", font=("Roboto", 14))
steps_entry.pack(side="right")
steps_entry.insert(0, "28")

steps = ctk.CTkSlider(group_steps, from_=1, to=100, number_of_steps=100, height=16, command=lambda value: steps_entry.configure(text=f"Steps: {int(value)}"))
steps.pack(fill="x", pady=2)
steps.set(28)
steps_entry.bind("<Return>", lambda e: entry_event(e, steps_entry, steps))
...
cfg_entry = ctk.CTkEntry(lbl_cfg, width=40, height=20, fg_color=COR_FRAME, border_width=0, justify="center", font=("Roboto", 14))
cfg_entry.pack(side="right")
cfg_entry.insert(0, "4.5")

cfg = ctk.CTkSlider(group_cfg, from_=0.5, to=30, number_of_steps=295, height=16, command=lambda value: cfg_entry.configure(text=f"CFG Scale: {round(value, 1)}"))
cfg.pack(fill="x", pady=2)
cfg.set(4.5)
cfg_entry.bind("<Return>", lambda e: entry_event(e, cfg_entry, cfg))
...
lora_card = ctk.CTkFrame(lora_group, fg_color=COR_FRAME, corner_radius=8)
...
model_lora = ctk.CTkButton(lora_bot, text="+", state="disabled", width=30, height=24, font=("Roboto", 14), command=lambda: select_lora(lora_listbox, model_lora, lora_label, lora_scale), fg_color=COR_BOTAO, hover_color=COR_BOTAO_HOVER)
...
chk_sequenc = ctk.CTkCheckBox(row_checks, text="Mult-Imagens (Experimental)", command=active_benchmark_sequence, font=("Roboto", 11), text_color="#DDDDDD", fg_color=COR_BOTAO, hover_color=COR_BOTAO_HOVER, corner_radius=4, height=20)
...
# Botão de Gerar
footer_dock = ctk.CTkFrame(window, height=80, fg_color=COR_FRAME)
footer_dock.pack(side="bottom", fill="x")
footer_dock.pack_propagate(False)
action_area = ctk.CTkFrame(footer_dock, fg_color=COR_INPUT, corner_radius=15)
...
generate_button = ctk.CTkButton(action_area, text="GERAR IMAGEM", command=lambda: viwerImage(generate_button, temperature_label, f"{width_entry.get()}x{height_entry.get()}", prompt_entry, negative_prompt_entry, steps, cfg, seed_entry, lora_listbox, qtdImg=qtdImg), state="disabled", font=("Roboto", 15, "bold"), fg_color=COR_BOTAO, hover_color=COR_BOTAO_HOVER, height=40, corner_radius=8)
